# Seed Keywords Script Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Create a database seeding script for AI agentic keywords to power ZenWatch's tech watch capabilities.

**Architecture:** Single Python script following the exact pattern of `seed_sources.py` - define keywords data, check for existence, insert if missing.

**Tech Stack:** Python, SQLAlchemy, existing database utilities from `app.database`

---

## Task 1: Create seed_keywords.py Script

**Files:**
- Create: `backend/app/scripts/seed_keywords.py`

**Step 1: Create the seed_keywords.py file**
...
```bash
cd backend && poetry run python -m app.scripts.seed_keywords
```
...
------------------------------------------------------------
  ...
  [AGENTS]
    - autogpt (weight: 2.5, ACTIVE)
    - crewai (weight: 2.5, ACTIVE)

  [COMPANIES]
    - openai (weight: 3.0, ACTIVE)
    - anthropic (weight: 3.0, ACTIVE)
    - huggingface (weight: 2.5, ACTIVE)
...
```
...
Expected: Script should skip all existing keywords and show "Keywords created: 0"

**Step 4: Commit the changes**

```bash
git add backend/app/scripts/seed_keywords.py
git commit -m "feat(backend): add seed_keywords.py for AI agentic keywords"
```
...
|------|-------------|-------|
| 1 | Create seed_keywords.py script | `backend/app/scripts/seed_keywords.py` |

**Total keywords to seed:** 24 keywords across 7 categories (frameworks, agents, models, concepts, companies, tools, general)
